...
public class Notify4BasePay {
    ...
    static BlockingQueue<Runnable> workQueue = new ArrayBlockingQueue<>(10000);
    private static final ThreadPoolExecutor threadPool = new ThreadPoolExecutor(1, Integer.MAX_VALUE, 1, TimeUnit.HOURS, workQueue, new ThreadFactory() {
        AtomicInteger counter = new AtomicInteger(0);

        @Override
        public Thread newThread(@NotNull Runnable r) {
            return new Thread(r, "pool_compute_amout_after_pay_" + counter.getAndIncrement());
        }
    });
    @Autowired
    ...
    public String createNotifyUrl(PayOrder payOrder, String backType) {
        ...
        paramMap.put("payOrderId", payOrder.getId() == null ? "" : payOrder.getId());           // 支付订单号
        ...
        paramMap.put("channelOrderNo", ""); // 渠道订单号
    ...
    public void doNotify(PayOrder payOrder) {
        ...
        doComputeAmountFlow(payOrder);
    }

    public void doComputeAmountFlow(PayOrder payOrder) {
        _log.info(">>>>>> PAY开始计算账户调增调减 <<<<<<");
        // 发起后台通知业务系统
        threadPool.execute(() -> {
            try {
                billService.computeAmountFlowById(payOrder);
            } catch (Exception e) {
                _log.error("PAY开始计算账户调增调减出错。 payOrderId={},sendMessage error.", payOrder != null ? payOrder.getId() : "", e);
            }
        });
        _log.info(">>>>>> PAY开始计算账户调增调减完成 <<<<<<");
    ...
    public JSONObject createNotifyInfo(PayOrder payOrder) {
        ...
        object.put("orderId", payOrder.getId());
