...
public class MySeq {
	...
	private static AtomicLong file_seq = new AtomicLong(0L);
	private static String file_seq_prefix = "F";
	private static AtomicLong cash_seq = new AtomicLong(0L);
	private static String cash_seq_prefix = "C";
	...
	public static String getFile() {
		return getSeq(file_seq_prefix, file_seq);
	}

	public static String getCash() {
		return getSeq(cash_seq_prefix, cash_seq);
	}

	private static String getSeq(String prefix, AtomicLong seq) {
		prefix += node;
		return String.format("%s%s%04d", prefix, DateUtil.getSeqString(), (int) seq.getAndIncrement() % 10000);
	}
