...
public class AmountUtil {
    ...
    /**
     * 在指定金额的基本随机增加101-145分。并返回增加后的金额
     *
     * @return
     */
    public static long randomSuffix(long amount) {
        long suffix = 101 + System.currentTimeMillis() % 44;
        if (suffix < 0) {
            suffix = 11;
        }
        long result = BigDecimalUtils.add(amount, suffix);
        if (result < amount) {
            result = amount;
        }
        return result;
