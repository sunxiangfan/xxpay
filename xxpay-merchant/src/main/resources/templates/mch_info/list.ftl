	...
	<body>
		...
		<script type="text/html" id="tpl">
			...
			<tr>
				...
				<td>
					{{# if(item.throughAuditing === 2){ }} <span style="color: red">未通过审核</span> {{#  } }}
					{{# if(item.throughAuditing === 1){ }} <span style="color: green">已审核</span> {{#  } }}
					{{# if(item.throughAuditing === 0){ }} <span style="color: red">未审核</span> {{#  } }}
				</td>
				<td>
		...
		<script>
			...
			layui.use(['paging', 'form'], function() {
                ...
                function viewForm(id) {
                    ...
                    $.get('/mch_info/view.html?id=' + id, null, function(form) {
				...
				function editForm(mchId) {
                    //本表单通过ajax加载 --以模板的形式，当然你也可以直接写在页面上读取
                    $.get('/mch_info/edit.html?id=' + mchId, null, function(form) {
                        addBoxIndex = layer.open({
                            type: 1,
                            title: '保存用户',
                            content: form,
