...
public class MchBankCardService {
    ...
    public Integer deleteByPrimaryKey(String id) {
        return mchBankCardMapper.deleteByPrimaryKey(id);
    }
    ...
    void setCriteria(MchBankCardExample.Criteria criteria, MchBankCard info) {
        if (info != null) {
            ...
            if (StringUtils.isNotBlank(info.getNumber())) criteria.andNumberLikeTo("%" + info.getNumber() + "%");
            if (StringUtils.isNotBlank(info.getAccountName()))
                criteria.andAccountNameLikeTo("%" + info.getAccountName() + "%");
