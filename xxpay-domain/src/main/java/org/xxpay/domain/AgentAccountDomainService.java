...
@Service
public class AgentAccountDomainService {
    ...
    public int decreaseCash(String mchId, MchBankCard mchBankCard, Long applyAmount, long mchServiceCharge, String cashOrderId) {
        ...
        Assert.isTrue(applyAmount >= 10000, "申请金额不能为空，且必须大于100！申请值为：" + AmountUtil.convertCent2Dollar(applyAmount + ""));
        Agent preMchInfo = agentMapper.selectByPrimaryKey(mchId);
...
//        Assert.isTrue(enableUseAmount.compareTo(todayCashing) >= 0, "超出今日可提现金额！今天上限：" + AmountUtil.convertCent2Dollar(enableUseAmount + "") + " , 今日已提现：" + AmountUtil.convertCent2Dollar(todayCashed + ""));
        ...
        Agent updateMchInfoItem = new Agent();
        ...
        AgentExample mchInfoExample = new AgentExample();
        AgentExample.Criteria mchInfoCriteria = mchInfoExample.createCriteria();
        ...
        int updateRows = agentMapper.updateByExampleSelective(updateMchInfoItem, mchInfoExample);
     ...
     */
    @Transactional
    public int audit(String agentId, MchWithdrawApply apply, boolean isPass, String auditor) {
        /**
         * 提现审核
         * 1.通过：
         *      1.1 抹去账户中申请中申请金额
        ...
        Agent preMchInfo = agentMapper.selectByPrimaryKey(agentId);
        Assert.notNull(preMchInfo, "代理信息不存在！mchId: " + agentId);
        Assert.isTrue(preMchInfo.getState().equals(MchState.ENABLE.getCode()), "代理已被停用！agentId: " + agentId);
        Long preCashingAmount = preMchInfo.getCashingAmount();
        Assert.isTrue(preCashingAmount.compareTo(applyAmount) >= 0, "代理累计待审批金额有误。待审批金额为：" + AmountUtil.convertCent2Dollar(preCashingAmount + "") + " 不能小于本次申请提现金额： " + AmountUtil.convertCent2Dollar(applyAmount + ""));
        ...
        AgentExample mchInfoExample = new AgentExample();
        AgentExample.Criteria mchInfoCriteria = mchInfoExample.createCriteria();
        mchInfoCriteria.andIdEqualTo(agentId);
        mchInfoCriteria.andVersionEqualTo(preVersion);
        int updateRows = agentMapper.updateByExampleSelective(updateMchInfoItem, mchInfoExample);
        ...
        if (!isPass) {
            ...
            amountFlowDomainService.rejectCashIncrease(agentId, preBalance, applyAmount, mchServiceCharge, cashOrderId);
        }
        return 1;
