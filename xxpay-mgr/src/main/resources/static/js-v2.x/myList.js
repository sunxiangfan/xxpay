layui.define(['form', 'table', 'common'], function (exports) {
    ...
    MyList.prototype.init = function () {
        var scope = this;
        var defaultToolFn = function (event, data, obj) {
            if ($.isFunction(scope[event])) {
                scope[event](data, obj);
            }
        }

        var defaultToolbarFn = function (event, datas, obj) {
            // if (datas.length <= 0) {
    ...
    MyList.prototype.initTable = function () {
        ...
        table.on('tool(table)', function (obj) {
            var event = obj.event;
            var data = obj.data;
            scope.config.toolFn(event, data, obj);
        });
        ...
        table.on('toolbar(table)', function (obj) {
            ...
            var event = obj.event;
            scope.config.toolbarFn(event, data, obj);
        });
        //监听状态操作
